...
Both client and server must mount this mod. A local Launchpad game does that for the first test. The user published the original one-badge prototype as v0.8 and confirmed v1.0 publication on 2026-09-16. Broader live visual and multiplayer compatibility checks remain to be recorded.
...
This is a real earned award used as the first test case, rather than an artificial grant of all badges. The published v1.0 build grants 119 distinct awards to 105 accounts. Made_in_Russia remains unassigned. Void and Reguliym have two evidenced roster entries pointing to one account award; the generator requires an explicit shared-award link and emits a single grant. Issues #1 and #2 track these outliers. [Recipient review](recipient-review.md) records the evidence, aliases, and outstanding questions. `scripts/generate_assignments.py` validates all account formats and generates the two runtime tables from `data/recipients.json`. S13 D1 bronze remains omitted under the accepted policy.
